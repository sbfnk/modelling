...
You can either work in a single R window, or use the user friendly interface RStudio, which allows you to have your saved R code, separate to your executed commands. It also holds the plot, environment and history window. Using RStudio you  can keep your model code (a 'file.R') separate to your executed code in the Console window. In this 'file.R' you can store the useful and checked code for future use. For example, you could start a new R file and store in it all the commands below.   
...
```{r results = "hide", eval = FALSE}
home <- "~/Imperial/Chennai_workshop/"
setwd(home)
```
...
Does this give you the output you expected? You will need to use some of the commands for looking at data that were introduced above. The file `ward.csv` can be found [here](https://github.com/sbfnk/chennai-modelling/raw/master/data/ward_data.csv).
...
Finally, the `ode` function requires the input parameters, which here are only the two beta and gamma:
```{r results = "hide"}
...
Does this match the data well? Note, that the data given is the proportions infected over time. So in order to compare you either need to convert the data to numbers or the numbers to data. Try and do this, and plot the data and model output on the same graph. A solution is given below if you get stuck. 
...
Given that you know that the value for `beta` is 2/100000, can you write a function that considers a range of different parameters for `gamma`, runs the model with these various values and then plots the outcomes with the data? From this what is the best estimate you can get for `gamma`? 
...
## Going further: the SIR model with births and deaths
...
## Going even further: extending the SIR model
