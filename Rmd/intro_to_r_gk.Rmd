...
```{r fitR, cache=FALSE, echo=FALSE, results = "hide"}
library('curl')
```
...
3. solve an SIR model in R
4. build an SEIR model and create your own model output. 

# Basic syntax

## Commenting code

Using RStudio we can keep our model code (our 'file.R') separate to our executed code in the Console window. Within R code, anything behind a '#' is "commented" out and won't be executed. It is good coding practice to comment your code as you go through. 
...
```
You can also label this working directory as 'home', and have other locations stored for plotting or for getting data. 

## Mathematical elements in R

To following symbols represent multiplication '*', addition '+', subtraction '-' and assignment of a name to an object '->'. To check if something is equal use "==", or not equal using "!=". 
...
For this practical, you will first need to download the required data from ??? Save it in the correct folder and make this your current working directory. 

There are many ways to read in data. For example, download the file `data.csv` from [here](https://github.com/sbfnk/chennai-modelling/raw/master/data/data.csv) and place it in your working directory. Here, as we have a '.csv' file we could use either of:
...
```
Here both 'mydata' and 'flu_hh' have the same data. Can you check this in R?
The data is read in in the form of a 'data.frame'. This is a type of data format in R that includes both numbers and strings. There is more information on reading in data at 'http://www.statmethods.net/input/importingdata.html'. 

In order to look at the data we have read in there are several useful tools. Copy the below code into your R session and check that you understand all the commands. 
...
How would you go about finding the maximum time in this data? At what time is the number of cases maximum? See if you can write code to do this yourself. To do this you may need to use the function 'which' that searches for the location of matching values. The answers to this are below.
...
To add another column to a data.frame you simply refer to the name of this new column and assign it to have certain values. For example:
...
You can also alter the size of the axis to focus on certain areas. This use the 'max_prop' from above to focus on the increase in the outbreak only.
...
dev.off() # magic plot command - if things break do this until error :) Don't ask me why. 
```
...
```{r results = "hide"}
write.table(flu_hh, "flu_hh.txt", sep="\t") 
write.csv(flu_hh,"flu_hh.csv")
```
...
Conditionals are built in functions that allow you to compare and contrast elements in R. 

For example, the 'which' function gives you the index of values that match some condition:
```{r results = "hide"}
...
x = ifelse( mean(flu_hh$prop) > 0.5, max(flu_hh$prop), 0)
```
...
INDENT: "command + I" on mac. highlight all code and enter. Helps find mistakes and lays out code nicely

COMMENT: "command + C" on mac. As above but makes all commented. Useful if you want to comment out a load of code and run without temporarily
...
Functions are useful ways of packaging up pieces of code. If you are going to do a set of commands multiple time then it can be simpler and cleaner to place them in a function that has been thoroughly checked and that you have not altered subsequently.
...
Does this give you the output you expected? 
...
In order to write and solve odes, you need to learng the basic syntax in R and load the package that we installed earlier (deSolve). Remember that to do this you need to type:
```{r results = "hide"}
library(deSolve) 
```
...
For this part of the practical, we will build and solve out own SIR model. We will compare it to data and try to determine the correct parameters. 

Firstly we need to read in the data, which is stored in "data_sir.csv". As before, download this from ?? and point R to the correct working directory where you have this data saved. Then you can type:
...
init <- c(S = 0, I = 0, R = 0)
```
Can you alter this initial vector to have initially 1/100000 proportion of the population infected and the rest susceptible? 
...
Finally, the ode requires the input parameters, which here are only the two beta and gamma:
```{r results = "hide"}
...
To look at all of the model output we can plot everything, including the data on the same graph. This requires us to use the "line" and "points" commands:
```{r results = "hide"}
plot(out[,"time"],out[,"S"],type="l",col="green",ylim=c(0,1),xlab="Time",ylab="Proportion")
lines(out[,"time"],out[,"I"],col="red")
lines(out[,"time"],out[,"R"],col="blue")
legend(30, 0.7, c("Susceptibles", "Infecteds", "Recovereds"), lty = 1, col = 2:4) # Add legend and specific position
# Compare to the data - plot this data on the same graph
points(data[,1],data[,2])
```

Does this match the data well? Given that you know that the value for 'beta' is 1.3, can you write a function that considers a range of different parameters for 'gamma', runs the model with these various values and then plots the outcomes with the data? From this what is the best estimate you can get for 'gamma'? 

```{r results = "hide"}

#*************************************************************** Writing and solving odes
#### Basic script to run SIR model 


# {{Seq build a vector for you - many ways to build vectors!}}

# Run the function using "ode", a function from the library we loaded earlier
## Plot all on the same axes

### Run for many values of gamma
gamma_values = seq(0.1,0.9,0.1) # vector of possible gamma values
...
for(i in 1:length(gamma_values)){ # Length = how many entries in vector
  ...
  # New parameter set
  parameters = c(beta = 1.4247, gamma = gamma_new)
  ...
  stored2 = cbind(out[,1],out[,3],gamma_new) # Bind the columns with time, the infected and the new gamma
  store_I[((i-1)*length(times)+1):(i*length(times)),]=stored # Binds them to the old data
}
...
# Plot! 
plot(store_I[,"Time"],store_I[,"Infecteds"],type="l",xlab="Time",ylab="Proportion")
points(data[,1],data[,2]) # compare visually to data

# Store the hard earned data somewhere so you don't have to run it again
```
